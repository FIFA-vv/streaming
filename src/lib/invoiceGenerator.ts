import { Invoice, PlanTier, BillingCycle } from '../types/subscription';
...
export function generateInvoice(
    ...
    userEmail: string = 'user@wetube.com',
    userName: string = 'WeTube User',
    paymentId: string = `pay_rzp_${Math.random().toString(36).substring(2, 10)}`,
    orderId: string = `order_${Math.random().toString(36).substring(2, 10)}`,
    paymentMethod: string = 'Razorpay Test (UPI / Card)'
): Invoice {
    const plan = SUBSCRIPTION_PLANS[planId];
    const now = new Date();
    const dateStr = now.toISOString().split('T')[0];

    // Calculate expiry date based on cycle
    ...
    // Calculate discount for yearly (already included in priceYearly, but let's show breakdown)
    ...
    // Tax calculation (Base price + 18% GST)
